// halkaan soo dhig koodhkaaga Dart ee faylka lib/screens/dashboard_screen.dart
...
class _DashboardScreenState extends State<DashboardScreen> {
          ...
          floatingActionButton: FloatingActionButton.extended(
            ...
            onPressed: () async {
              final res = await Navigator.push(
                context,
                MaterialPageRoute(builder: (c) => AddCropScreen()),
              );
  ...
  Widget _buildHeroStats(bool isDesktop) {
    ...
    return Wrap(
      ...
      children: [
        _statCard(
          _userRole == "Admin" ? "Global Crops" : "My Crops",
          "${_allCrops.length}",
          Icons.eco,
          Colors.green,
        ),
        ...
        // --- TOTAL FIELDS ---
        _statCard(
          _userRole == "Admin" ? "Total Fields" : "My Fields",
          "${myFields.length}",
          Icons.map,
          Colors.orange,
        ),
        ...
        _statCard(
          _userRole == "Admin" ? "System Value" : "My Assets",
          ...
          children: [
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    _isWeatherLoading ? "--°C" : "${_temp.toInt()}°C",
                    style: const TextStyle(
                      color: Colors.white,
                      fontSize: 24,
                      fontWeight: FontWeight.bold,
            ...
            Icon(
              _condition.contains("Cloud") ? Icons.cloud : Icons.wb_sunny,
              color: Colors.yellow,
              size: 40,
            ),
          ],
        ...
        if (canManage)
          IconButton(
            ...
            onPressed: () async {
              final res = await Navigator.push(
                context,
                MaterialPageRoute(builder: (c) => EditCropScreen(crop: crop)),
              );
              if (res == true) _refreshData();
            },
          ),
        ...
        children: [
          ...
          _sidebarItem(
            ...
            onTap: () async {
              final result = await Navigator.push(
                context,
                MaterialPageRoute(builder: (c) => FieldsScreen()),
              );
              if (result == true) _refreshData();
            },
          ),
          ...
          _sidebarItem(
            ...
            onTap: () => Navigator.push(
              context,
              MaterialPageRoute(builder: (c) => AnalyticsScreen()),
            ),
          ),
          _sidebarItem(
            Icons.person,
